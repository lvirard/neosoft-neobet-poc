  ...
  const filteredCollaborators= computed(() => {
    return allCollaborators.value.filter(p => {
      ...
      const matchTitle = !filters.value.title
        || p.title === filters.value.title;

      const matchOffice = !filters.value.office
        || p.office.name === filters.value.office;

      const matchSeniority = !filters.value.seniority
        || p.seniority === filters.value.seniority;

      const matchIsAvailable = !filters.value.isAvailable 
        || p.isAvailable === true;

      return matchSearch && matchTitle && matchOffice && matchSeniority && matchIsAvailable;
    })
  })
